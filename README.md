...
> **This branch is a test harness, not the project template.**
> `feature/workflow-transition-request` wires this skeleton to the matching branch of
> [Prokyonn/sulu](https://github.com/Prokyonn/sulu) so the "request for publish" review flow can be
> clicked through end to end. Start at **[TESTING-request-for-publish.md](TESTING-request-for-publish.md)**
> for setup and a 23-step script, and **[DESIGN-request-for-publish.html](DESIGN-request-for-publish.html)**
> for the walkthrough with screenshots. `composer.lock` is committed here on purpose, so everyone
> installs the same `sulu/sulu` commit.
